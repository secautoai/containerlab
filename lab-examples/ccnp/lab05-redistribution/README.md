...
| | |
| --- | --- |
| Blueprint mapping | **ENARSI 1.1** (administrative distance), **1.2** (route maps), **1.3** (loop prevention: filtering/tagging), **1.4** (redistribution), **1.5** (summarization), **ENARSI 4.5 & ENCOR 4.4** (IP SLA + object tracking), PBR |
...
And a semantics point the exam loves: packets that match **no** clause (or a *deny* clause) are
**not dropped** — they simply fall through to normal destination-based routing. (That implicit
pass-through is the opposite of route-maps used for redistribution, where the implicit deny
discards routes.)
...
```
! r1
ip route 10.0.24.4 255.255.255.255 10.0.12.2    ! pin the probe target to the r2 path
!
ip sla 1
...
Two deliberate subtleties here. First, why probe `10.0.24.4` (r4's address on the r2–r4 link)
and not the final destination? The probe must test the **primary path specifically** — probing
198.51.100.1 would start succeeding over the backup path after failover and flap the track.
Second, why the `/32` static? `source-interface` only sets the probe's *source address*, not
its egress: r1 learns 10.0.24.0/24 as an EIGRP external via **both** borders, so without the
host route the probe could happily ECMP via r3 and keep the track Up while the r2 path is dead.
The longest-match /32 forces every probe through r2 (and it self-cleans: if the r1–r2 link
itself dies, the /32's next hop becomes unresolvable and the route drops out with it). `delay
...
Final configs in [`solutions/`](solutions/); deploy with `./deploy.sh reset 5 && ./deploy.sh deploy 5 --solved`.
</details>
